...
export default function ManufacturerDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch latest scan events from products
      let allScans = [];
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 font-mono">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#0D121A] p-6 rounded-xl border border-[#1E293B] shadow-xl">
        <div className="space-y-1">
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-cyan-950/60 border border-cyan-500/30 text-cyan-400 text-xs font-medium">
            ...
            <span>VERIFYX OPERATIONAL DASHBOARD</span>
          </div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Company Operations & Supply Chain Hub</h1>
          <p className="text-xs text-[#94A3B8]">
            Real-time tracking of order creation, QR box printing, packaging, quality control, and deliveries.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3 text-xs">
        <div className="bg-[#0D121A] p-4 rounded-xl border border-[#1E293B] space-y-1">
          <span className="text-[#94A3B8] text-[10px] block uppercase font-semibold">Total Orders</span>
          <div className="text-2xl font-bold text-white">{stats.totalOrders}</div>
          <span className="text-[10px] text-cyan-400">Sales Orders</span>
        </div>

        <div className="bg-[#0D121A] p-4 rounded-xl border border-[#1E293B] space-y-1">
          <span className="text-[#94A3B8] text-[10px] block uppercase font-semibold">Processing</span>
          <div className="text-2xl font-bold text-amber-400">{stats.processing}</div>
          <span className="text-[10px] text-[#94A3B8]">Assigned & QR</span>
        </div>

        <div className="bg-[#0D121A] p-4 rounded-xl border border-[#1E293B] space-y-1">
          <span className="text-[#94A3B8] text-[10px] block uppercase font-semibold">Packed</span>
          <div className="text-2xl font-bold text-cyan-400">{stats.packed}</div>
          <span className="text-[10px] text-[#94A3B8]">Quality Checked</span>
        </div>

        <div className="bg-[#0D121A] p-4 rounded-xl border border-[#1E293B] space-y-1">
          <span className="text-[#94A3B8] text-[10px] block uppercase font-semibold">Dispatched</span>
          <div className="text-2xl font-bold text-indigo-400">{stats.dispatched}</div>
          <span className="text-[10px] text-[#94A3B8]">Handed Courier</span>
        </div>

        <div className="bg-[#0D121A] p-4 rounded-xl border border-[#1E293B] space-y-1">
          <span className="text-[#94A3B8] text-[10px] block uppercase font-semibold">In Transit</span>
          <div className="text-2xl font-bold text-blue-400">{stats.inTransit}</div>
          <span className="text-[10px] text-[#94A3B8]">Hub Checkpoints</span>
        </div>

        <div className="bg-[#0D121A] p-4 rounded-xl border border-[#1E293B] space-y-1">
          <span className="text-[#94A3B8] text-[10px] block uppercase font-semibold">Delivered</span>
        ...
        <div className="bg-[#0D121A] p-4 rounded-xl border border-red-500/40 space-y-1">
          <span className="text-red-400 text-[10px] block uppercase font-semibold">Issues</span>
          <div className="text-2xl font-bold text-red-400">{stats.issues}</div>
          <span className="text-[10px] text-red-300">Requires Attention</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 bg-[#0D121A] p-6 rounded-xl border border-[#1E293B] space-y-4 shadow-xl">
          <div className="flex items-center justify-between border-b border-[#1E293B] pb-3">
            ...
            <Link to="/orders" className="text-xs text-cyan-400 hover:underline">
              View All Orders ➔
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {recentOrders.map((ord) => (
              <div key={ord._id} className="p-3.5 rounded-lg bg-[#111821] border border-[#1E293B] space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-cyan-400">{ord.orderId}</span>
                  <span className="text-[10px] px-2 py-0.5 rounded bg-cyan-950 text-cyan-300 border border-cyan-500/30">
                    {ord.status}
                  </span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-white font-semibold">{ord.productName}</span>
                  <span className="text-[#94A3B8]">{ord.customerName}</span>
                </div>
                <div className="pt-1 flex justify-between items-center text-[11px] text-[#94A3B8]">
                  <span>Date: {new Date(ord.orderDate).toLocaleDateString()}</span>
                  <Link to={`/orders/${ord.orderId}`} className="text-cyan-400 hover:underline">
        ...
        {/* Recent Scan Events Audit Stream */}
        <div className="lg:col-span-6 bg-[#0D121A] p-6 rounded-xl border border-[#1E293B] space-y-4 shadow-xl">
          <div className="flex items-center justify-between border-b border-[#1E293B] pb-3">
            <h3 className="text-sm font-bold text-white flex items-center space-x-2">
              <Clock className="w-4 h-4 text-emerald-400" />
              <span>Live Scan Events Audit Stream</span>
            </h3>
          ...
          <div className="space-y-3">
            {recentScans.map((s) => (
              <div key={s._id} className="p-3.5 rounded-lg bg-[#111821] border border-[#1E293B] space-y-1.5 text-xs">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-white">{s.productId}</span>
                  <span className="text-[10px] font-bold text-cyan-300 bg-cyan-950 px-2 py-0.5 rounded border border-cyan-500/30">
                    {s.stage}
                  </span>
                </div>
                <div className="text-[#94A3B8] flex justify-between text-[11px]">
                  <span>Location: <strong className="text-cyan-300">{s.location}</strong></span>
                  <span>Op: {s.employeeName}</span>
                </div>
