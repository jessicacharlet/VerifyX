...
export default function OrderDetailPage() {
  ...
  if (loading) {
    return <div className="p-12 text-center text-xs font-sans text-[#94A3B8]">Loading order details...</div>;
  }
  ...
  if (error || !order) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-12 text-center space-y-4 font-sans">
        ...
        <div className="text-base font-semibold text-white">Order Not Found</div>
      ...
      <div className="bg-[#0D121A] p-6 rounded-xl border border-[#1E293B] shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center space-x-2">
            <span className="text-xl font-semibold font-mono text-cyan-400">{order.orderId}</span>
            <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-cyan-950/80 text-cyan-300 border border-cyan-500/30">
              {order.status}
            </span>
          </div>
          <h1 className="text-lg font-semibold text-white">{order.productName}</h1>
          <div className="text-xs text-[#94A3B8]">
            Customer: <strong className="text-slate-200 font-medium">{order.customerName}</strong> • Date: {new Date(order.orderDate).toLocaleDateString()}
          </div>
        </div>
        ...
        <div className="flex gap-2">
          {order.assignedProducts && order.assignedProducts.length > 0 && (
            <Link
              ...
              className="px-4 py-2.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-[#070A0F] font-semibold text-xs flex items-center space-x-2 transition-all shadow-sm"
            >
          ...
          <div className="bg-[#0D121A] p-6 rounded-xl border border-[#1E293B] space-y-4">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="p-3 rounded bg-[#111821] space-y-1 border border-[#1E293B]">
                ...
                <span className="text-slate-200 block">{order.customerAddress}</span>
              </div>
          ...
          <div className="bg-[#0D121A] p-6 rounded-xl border border-[#1E293B] space-y-4">
            ...
            {order.assignedProducts && order.assignedProducts.length > 0 ? (
                ...
                {order.assignedProducts.map((prod) => (
                  <div key={prod._id || prod} className="p-4 rounded-lg bg-[#111821] border border-cyan-500/30 space-y-3">
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-sm font-semibold text-white">{prod.productName || order.productName}</div>
                        <div className="text-xs font-mono font-semibold text-cyan-400 mt-0.5">
                          Product ID: {prod.productId || prod}
                        </div>
                      </div>
                      <span className="text-xs px-2.5 py-0.5 rounded-full bg-emerald-950 text-emerald-400 border border-emerald-500/30 font-medium">
                        {prod.currentStage || "ASSIGNED"}
                      </span>
                    </div>

                    <div className="text-xs text-[#94A3B8] space-y-1">
                      <div>Serial number: <span className="font-mono text-slate-200">{prod.serialNumber || "SN-AUTO-ASSIGNED"}</span></div>
                      <div>Batch: <span className="font-mono text-slate-200">{prod.batchNumber || "BATCH-2026-A1"}</span></div>
                      <div>Current location: <span className="text-cyan-300 font-medium">{prod.currentLocation || "Warehouse"}</span></div>
                    </div>

                    <div className="pt-2 flex gap-2">
                      <Link
                        to={`/products/${prod.productId || prod}`}
                        className="px-3 py-1.5 rounded bg-[#0D121A] hover:bg-[#1E293B] text-xs text-cyan-400 border border-[#1E293B] inline-flex items-center space-x-1 font-medium"
                      >
                      ...
                      <Link
                        ...
                        className="px-3 py-1.5 rounded bg-cyan-400 hover:bg-cyan-300 text-[#070A0F] font-semibold text-xs inline-flex items-center space-x-1"
                      >
              ...
              <div className="p-6 text-center rounded bg-[#111821] border border-[#1E293B] space-y-2">
                ...
                <div className="text-xs font-semibold text-amber-400">NO PHYSICAL PRODUCT ASSIGNED</div>
        ...
        <div className="lg:col-span-6">
          <div className="bg-[#0D121A] p-6 rounded-xl border border-cyan-500/40 space-y-5 shadow-2xl relative">
            ...
            <form onSubmit={handleAssignSubmit} className="space-y-4 text-xs">
              <div className="space-y-1">
                <label className="text-slate-300 block font-medium">Serial number (unique box ID):</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-slate-300 block font-medium">Batch number:</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1">
                  <label className="text-slate-300 block font-medium">Brand name:</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-slate-300 block font-medium">IMEI / unique identifier:</label>
                  <input
                    type="text"
                    value={assignForm.imei}
                    onChange={(e) => setAssignForm({ ...assignForm, imei: e.target.value })}
                    placeholder="e.g. 359821098234123"
                    className="w-full px-3 py-2 rounded bg-[#111821] border border-[#1E293B] text-white focus:outline-none focus:border-cyan-400 font-mono"
                  />
                </div>

                <div className="space-y-1">
                  <label className="text-slate-300 block font-medium">Warehouse location:</label>
                  <input
                    type="text"
              ...
              <button
                ...
                className="w-full py-3.5 rounded-lg bg-cyan-400 hover:bg-cyan-300 text-[#070A0F] font-semibold text-xs flex items-center justify-center space-x-2 transition-all shadow-md shadow-cyan-500/20"
              >
                <QrCode className="w-4 h-4" />
