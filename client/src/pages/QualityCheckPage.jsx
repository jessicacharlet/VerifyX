    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 font-sans">
      ...
      <div className="bg-[#0D121A] p-6 rounded-xl border border-[#1E293B] shadow-lg flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl font-semibold text-white tracking-tight">Quality Check Inspection Queue</h1>
      ...
      {loading ? (
        ...
        <div className="p-12 text-center bg-[#0D121A] rounded-xl border border-[#1E293B] space-y-2">
          ...
          <div className="text-sm font-semibold text-white">QC Queue Empty</div>
          ...
          {products.map((prod) => (
            <div key={prod._id} className="bg-[#0D121A] p-5 rounded-xl border border-[#1E293B] space-y-4 shadow-xl relative">
              <div className="flex items-center justify-between border-b border-[#1E293B] pb-3">
                <span className="text-xs font-mono font-semibold text-cyan-400">{prod.productId}</span>
              ...
              <div className="space-y-1">
                <div className="text-sm font-semibold text-white">{prod.productName}</div>
                <div className="text-xs text-[#94A3B8]">Serial: <span className="font-mono text-slate-200">{prod.serialNumber}</span></div>
                <div className="text-xs text-[#94A3B8]">Order: <span className="font-mono text-slate-200">{prod.orderId || "N/A"}</span></div>
              </div>

              <div className="p-3 rounded bg-[#111821] border border-[#1E293B] text-xs space-y-1">
                <div className="text-[#94A3B8]">Location: <span className="text-white font-medium">{prod.currentLocation}</span></div>
                <div className="text-[#94A3B8]">Condition: <span className="text-emerald-400 font-medium">{prod.condition}</span></div>
              </div>

              <Link
                to={`/scan?id=${prod.productId}`}
                className="w-full py-2.5 rounded bg-emerald-500 hover:bg-emerald-400 text-[#070A0F] font-semibold text-xs flex items-center justify-center space-x-1.5 transition-all"
              >
